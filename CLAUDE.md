# ontology-agent (Mini AIP)

A minimal, self-hostable take on **Palantir AIP** — a platform that connects LLMs and AI agents
to an organization's internal data and business workflows *safely*, by routing every interaction
through an **Ontology** (the semantic model of business objects and their relationships) rather
than over raw documents or tables.

> **Scope definition (agreed with the user):**
> `Mini AIP = Ontology + RAG + Permission + Action execution + Audit log`
>
> - **Ontology** — business concepts as `ObjectType` / `LinkType` / `ActionType` (e.g. Aircraft —has→ Flight —triggers→ MaintenanceEvent). The semantic core, not just a schema.
> - **RAG** — retrieval grounded in ontology objects, not free-floating documents.
> - **Permission** — object/row/column-level access control applied before the LLM ever sees data.
> - **Action execution** — the agent invokes governed `ActionType`s (writeback) instead of mutating data directly.
> - **Audit log** — every query, retrieval, permission decision, and action is recorded.
>
> This is the Ontology + AIP (agent) layer of Foundry, reproduced at small scale. A minimal
> data store stands in for Foundry's data-integration layer.

## Status

Greenfield. **Design-first** (the user wants design documents before implementation).
No application code exists yet — do not document a file structure or commands that have not been built.

## Intended Stack

- **Language / framework**: Python — FastAPI + Pydantic
  - Ontology object/link/action types are modeled as Pydantic models.
  - FastAPI exposes the Ontology API; the agent layer calls it via tool calling.
- **LLM**: Claude (Anthropic) — default to the latest capable model when wiring the agent layer.

> The stack above is the agreed direction, not yet realized in code. Confirm against actual
> files before relying on any path. Update this section as the project is built.

## How this project is run

This repo follows the **AI-DLC** workflow defined below and in `.aidlc-rule-details/`.
Work proceeds through INCEPTION → CONSTRUCTION → OPERATIONS phases with approval gates.
Planning/design artifacts live in `aidlc-docs/`; application code lives at the workspace root.
...
**CRITICAL**: When performing any phase, you MUST read and use relevant content from rule detail files in `.aidlc-rule-details/`.
